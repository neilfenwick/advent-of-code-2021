...
// Calculates whether a point is on a line by checking if the
...
func consume(worker int, m *sparseVentMap, jobs <-chan *point, result chan<- int) {
	var count int
...
func (m *sparseVentMap) CountVentIntersectionsOverThreshold(threshold int) int {
	var (
		count int
	)
	...
	for i := 1; i <= workerCount; i++ {
		go consume(i, m, pointsChannel, results)
	}
